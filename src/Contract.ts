...
const log = debug("alveyjs:contract")

const { logDecoder } = require("alveyjs-ethjs-abi") as IETHABI
...
export interface IContractSendRequestOptions {
  /**
   * The amount in ALV to send. eg 0.1, default: 0
   */
  ...
  /**
   * Alvey price per gas unit, default: 0.00000001, min:0.00000001
   */
...
export class Contract {
   ...
   *      [solar](https://github.com/alveyproject/solar). It includes the contract
   *      address, owner address, and ABI definition for methods and types.
   * @param opts - init options
   */
  constructor(
    private rpc: AlveyRPC,
  ...
  /**
   * Executes contract method on your own local alveyd node as a "simulation"
